...
<nav class="fixed inset-x-0 bottom-0 z-[100] border-t border-cyan-100 bg-white/95 shadow-[0_-8px_30px_rgba(15,23,42,.10)] backdrop-blur-md sm:hidden" aria-label="Mobile navigation">
    <ul class="mx-auto grid max-w-xl grid-cols-5">
        <li>
            <a href="{{ url('/') }}" class="flex flex-col items-center gap-1 px-1 py-2 text-slate-500 transition hover:text-cyan-600">
        ...
        <li>
            <a href="{{ route('shop') }}" class="flex flex-col items-center gap-1 px-1 py-2 text-slate-500 transition hover:text-cyan-600">
        ...
        <li>
            <a href="{{ route('cart') }}" class="relative flex flex-col items-center gap-1 px-1 py-2 text-slate-500 transition hover:text-cyan-600">
                <span class="footer-nav-icon relative flex h-8 w-8 items-center justify-center rounded-xl bg-cyan-50">
                    <i class="fa-solid fa-cart-shopping text-sm"></i>
                    <span id="footerCartCount" class="footer-pulse absolute -right-2 -top-2 flex h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white shadow-sm">{{ $footerCartCount }}</span>
                </span>
                <span class="text-[11px] font-semibold">Cart</span>
            </a>
        </li>
        <li>
            <a href="{{ route('profile') }}" class="flex flex-col items-center gap-1 px-1 py-2 text-slate-500 transition hover:text-cyan-600">
                @if(auth()->check() && auth()->user()->profile_image)
                    <img src="{{ asset('uploads/'.auth()->user()->profile_image) }}" class="h-8 w-8 rounded-xl border border-cyan-900/10 object-cover" alt="Profile">
                @else
    ...
    <div class="mx-auto max-w-7xl px-5 py-12 sm:px-6 lg:px-8 lg:py-14">
        <div class="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="lg:col-span-1">
                <a href="{{ url('/') }}" class="mb-5 inline-flex items-center gap-3">
                   @if($footerLogo)
    @php
...
@else
                    ...
                    @endif
                    <span class="max-w-[190px] text-lg font-extrabold leading-tight text-white">{{ $footerName }}</span>
                </a>
                <p class="max-w-sm text-sm leading-6 text-slate-400">
                    {{ $footerAbout ? \Illuminate\Support\Str::limit(strip_tags($footerAbout), 190) : 'Professional biomedical equipment and healthcare solutions with dependable service and customer support.' }}
                </p>
                <div class="mt-6 flex items-center gap-2">
                    @if($footerCompany?->facebook)
                        <a href="{{ $footerCompany->facebook }}" target="_blank" rel="noopener noreferrer" aria-label="Facebook" class="footer-social footer-anim flex h-10 w-10 items-center justify-center rounded-xl border border-cyan-900/40 bg-[#0b2538] text-slate-200 transition hover:border-cyan-400 hover:bg-cyan-600 hover:text-white"><i class="fa-brands fa-facebook-f"></i></a>
                    @endif
                    @if($footerCompany?->youtube)
                        <a href="{{ $footerCompany->youtube }}" target="_blank" rel="noopener noreferrer" aria-label="YouTube" class="footer-social footer-anim flex h-10 w-10 items-center justify-center rounded-xl border border-cyan-900/40 bg-[#0b2538] text-slate-200 transition hover:border-rose-400 hover:bg-rose-600 hover:text-white"><i class="fa-brands fa-youtube"></i></a>
                    @endif
                    @if($footerPhone)
                        <a href="tel:{{ preg_replace('/[^0-9+]/', '', $footerPhone) }}" aria-label="Call us" class="footer-social footer-anim flex h-10 w-10 items-center justify-center rounded-xl border border-cyan-900/40 bg-[#0b2538] text-slate-200 transition hover:border-emerald-400 hover:bg-emerald-600 hover:text-white"><i class="fa-solid fa-phone"></i></a>
                    @endif
                    @if($footerEmail)
                        <a href="mailto:{{ $footerEmail }}" aria-label="Email us" class="footer-social footer-anim flex h-10 w-10 items-center justify-center rounded-xl border border-cyan-900/40 bg-[#0b2538] text-slate-200 transition hover:border-cyan-400 hover:bg-cyan-600 hover:text-white"><i class="fa-solid fa-envelope"></i></a>
                    @endif
                </div>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-3 text-sm">
                    <li><a href="{{ url('/') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>Home</a></li>
                    <li><a href="{{ route('shop') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>All Products</a></li>
                    <li><a href="{{ route('cart') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>Shopping Cart</a></li>
                    <li><a href="{{ route('hospital.services') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>Hospital Bio Medical Service</a></li>
                    <li><a href="{{ route('other.services') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>Other Service</a></li>
                    <li><a href="{{ route('profile') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>Company Profile</a></li>
                    <li><a href="{{ route('contact') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>Contact Us</a></li>
                    <li><a href="{{ auth()->check() ? url('/account') : route('login') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>My Account</a></li>
                    <li><a href="{{ route('login') }}" class="inline-flex items-center gap-2 transition hover:text-white"><i class="fa-solid fa-chevron-right text-[9px] text-cyan-400"></i>Login</a></li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-4 text-sm">
                    @if($footerPhone)
                        <li class="flex items-start gap-3"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#0b2538] text-cyan-300"><i class="fa-solid fa-phone"></i></span><div><p class="text-xs text-slate-500">Call us</p><a href="tel:{{ preg_replace('/[^0-9+]/', '', $footerPhone) }}" class="font-semibold text-slate-200 hover:text-white">{{ $footerPhone }}</a></div></li>
                    @endif
                    @if($footerEmail)
                        <li class="flex items-start gap-3"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#0b2538] text-cyan-400"><i class="fa-solid fa-envelope"></i></span><div><p class="text-xs text-slate-500">Email</p><a href="mailto:{{ $footerEmail }}" class="break-all font-semibold text-slate-200 hover:text-white">{{ $footerEmail }}</a></div></li>
                    @endif
                    @if($footerAddress)
                        <li class="flex items-start gap-3"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#0b2538] text-emerald-400"><i class="fa-solid fa-location-dot"></i></span><div><p class="text-xs text-slate-500">Visit us</p><p class="leading-5 text-slate-200">{{ $footerAddress }}</p></div></li>
                    @endif
                </ul>
            </div>
    ...
    <div class="border-t border-cyan-900/40 bg-[#03111c]/60">
        <div class="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-5 text-xs text-slate-500 sm:px-6 md:flex-row md:items-center md:justify-between lg:px-8">
            <p>© {{ date('Y') }} <span class="font-semibold text-slate-200">{{ $footerName }}</span>. All rights reserved.</p>
