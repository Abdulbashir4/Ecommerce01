...
<div class="flex pt-16">
    <aside id="sidebar" class="admin-scroll fixed inset-y-16 left-0 z-40 w-[min(18rem,88vw)] -translate-x-full overflow-y-auto bg-slate-950 p-4 text-slate-200 shadow-2xl transition-transform duration-300 md:sticky md:top-16 md:h-[calc(100vh-4rem)] md:translate-x-0 md:shadow-none">
        <nav class="space-y-2">
            ...
            <div id="productsMenu" class="ml-4 space-y-1 border-l border-white/10 pl-3 {{ request()->is('admin/products*') || request()->is('admin/catalog*') || request()->is('admin/product-importer*') ? '' : 'hidden' }}">
                ...
                @if(auth()->user()->hasPermission('products.create'))<a href="{{ route('admin.product-importer.index') }}" class="block rounded-lg px-3 py-2 text-sm font-bold transition {{ request()->is('admin/product-importer*') ? 'bg-gradient-to-r from-fuchsia-600/90 to-purple-500/90 text-white shadow-lg shadow-fuchsia-950/20' : 'hover:bg-white/10' }}"><i class="fa-solid fa-wand-magic-sparkles mr-2"></i>Amazon Demo Importer</a>@endif
                @if(auth()->user()->hasPermission('products.edit'))<a href="{{ route('admin.products.image-editor') }}" class="block rounded-lg px-3 py-2 text-sm transition hover:bg-white/10"><i class="fa-solid fa-wand-magic-sparkles mr-2"></i>Image Editor</a>@endif
            ...
            @if(auth()->user()->hasPermission('settings.general.view'))
            <a href="{{ route('admin.settings') }}" class="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold transition hover:bg-white/10">
