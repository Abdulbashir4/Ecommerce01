...
<div class="mx-auto max-w-6xl space-y-6">
    ...
    <form method="POST" action="{{ route('admin.settings.general.update') }}" class="space-y-6">@csrf
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <label class="flex items-center justify-between rounded-2xl border border-amber-200 bg-amber-50 p-4"><span><strong class="block">Maintenance Mode</strong><small class="text-xs text-slate-600">Public visitors will see the maintenance page while administrators can still access the admin panel.</small></span><span><input type="hidden" name="maintenance" value="0"><input type="checkbox" name="maintenance" value="1" @checked($settings['maintenance']) class="h-5 w-5 rounded border-slate-300 text-amber-600"></span></label>
