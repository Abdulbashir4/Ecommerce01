    ...
    <div class="relative mx-auto flex min-h-screen w-full max-w-7xl items-center justify-center px-4 py-8 sm:px-6 lg:px-8">
        <div class="grid w-full max-w-5xl overflow-hidden rounded-[2rem] border border-white/10 bg-white shadow-2xl shadow-black/30 lg:grid-cols-2">
            <section class="hidden bg-gradient-to-br from-cyan-950 via-blue-950 to-indigo-950 p-10 text-white lg:flex lg:flex-col lg:justify-between xl:p-12">
                <div>
                    <a href="<?php echo e(url('/')); ?>" class="inline-flex items-center gap-3 text-white no-underline">
            ...
            <main class="bg-white px-5 py-8 sm:px-10 sm:py-12">
                <div class="mx-auto max-w-md">
                    <div class="flex items-center justify-between">
                        <a href="<?php echo e(url('/')); ?>" class="inline-flex items-center gap-2 text-sm font-bold text-slate-500 transition hover:text-slate-900"><i class="fa-solid fa-arrow-left"></i> Store</a>
                        <a href="<?php echo e(url('/login')); ?>" class="text-sm font-extrabold text-blue-700 hover:text-blue-900">Sign in</a>
                    </div>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($errors->any()): ?>
                        <div class="mt-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700" role="alert">
                            <div class="flex items-start gap-3"><i class="fa-solid fa-circle-exclamation mt-0.5"></i><div class="space-y-1"><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?><p><?php echo e($error); ?></p><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?></div></div>
                        </div>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>

                    <form method="POST" action="<?php echo e(url('/register')); ?>" class="mt-7 space-y-5" id="registerForm">
                        <?php echo csrf_field(); ?>
                        <div>
                            <label for="name" class="mb-2 block text-sm font-extrabold text-slate-700">Full name</label>
                            <div class="relative"><i class="fa-regular fa-user pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i><input id="name" name="name" type="text" value="<?php echo e(old('name')); ?>" autocomplete="name" required maxlength="150" placeholder="Your full name" class="h-13 w-full rounded-2xl border border-slate-200 bg-slate-50 px-11 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-cyan-500 focus:bg-white focus:ring-4 focus:ring-cyan-500/10"></div>
                        </div>
                        <div>
                            <label for="phone" class="mb-2 block text-sm font-extrabold text-slate-700">Phone number</label>
                            <div class="relative"><i class="fa-solid fa-phone pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i><input id="phone" name="phone" type="tel" inputmode="tel" value="<?php echo e(old('phone')); ?>" autocomplete="tel" required maxlength="50" placeholder="01XXXXXXXXX" class="h-13 w-full rounded-2xl border border-slate-200 bg-slate-50 px-11 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-cyan-500 focus:bg-white focus:ring-4 focus:ring-cyan-500/10"></div>
                        </div>
                        <div>
...
<?php /**PATH E:\Laravel Project\optimumbiomedical\resources\views/auth/register.blade.php ENDPATH**/ ?>